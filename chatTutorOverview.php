...
// Generate the table
$output = '<h3>Overview of your Conversations '. $tid .'</h3>';
...
while ($student_row = $result_fetch_students->fetch_assoc()) {
    ...
    $output .= '<td><button class="btn btn-success reply-btn">Reply</button></td>';
    $output .= '</tr>';
}
...
$(document).ready(function(){
    $(".message-history-btn").click(function(){
        ...
        $.ajax({
            url: "chatTutorBackend.php",    // tutorReplyForm.php, chatSendMessageTutor.php, chatTutorBackend.php
            type: "POST",
